...
class PDF extends FPDF
{
    function Header()
    {
        ...
        $this->Cell(200, 8, 'DATA POSISI JABATAN', 0, 1, 'C');
...
$pdf->SetFont('Arial', '', 8);
$no = 1;
$query = mysqli_query($koneksi, "SELECT * FROM tbl_jabatann ORDER BY jabatan ASC");
while ($data = mysqli_fetch_array($query)) {
    ...
    $pdf->Cell(10, 10, $no, 1, 0, 'C');
    $pdf->Cell(35, 10, $data['jabatan'], 1, 0, 'L');
    $pdf->Cell(30, 10, $data['jenis'], 1, 0, 'L');
    $pdf->Cell(30, 10, $data['kategori'], 1, 0, 'L');
    $pdf->Cell(30, 10, $data['barang'], 1, 0, 'L');
    $pdf->Cell(30, 10, $data['kondisi'], 1, 0, 'L');
    $pdf->Cell(25, 10, $data['keterangan'], 1, 1, 'L');
    $no++;
}

$pdf->Output('I', 'Daftar_Posisi_Jabatan.pdf');
?>